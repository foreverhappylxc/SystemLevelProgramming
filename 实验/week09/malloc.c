...
//检查已分配的堆的函数
void check_allocated_heap(Tree* root)
{
	if (root!=NULL) {
		unsigned int p;
		unsigned int min;
		unsigned int max;
		min = (int)root + sizeof(Tree);
		max = min + root->size-1;
		for (p = min; p < max;p=p+4)
		{
			if (*(int*)p % 4 != 0)
				continue;
			if (*(int*)p<dseg_lo || *(int*)p>dseg_hi)
				continue;
			temp = contains(*(int*)p, (Tree**)(dseg_lo + sizeof(long)));
			if (temp) {
				temp->size = temp->size | 0x01;
				check_allocated_heap(temp);
			}
		}
	}
}
...
for( i=0; i<3; i++)
	{
		if(regs[i] % 4 != 0)
			continue;
		if(regs[i] < dseg_lo || regs[i] > dseg_hi)
			continue;
		temp = contains(regs[i],(Tree **)(dseg_lo + sizeof(long)));
		alloc_tree = * (Tree **) (dseg_lo + sizeof(long));
		if (temp) {
			temp->size = temp->size | 0x01;
			check_allocated_heap(temp);
   ...
   */
	...
	top = * (int *) top;
	tmp_t = * (int *) top;
	for(i = top; i < tmp_t; i += 4)
	{
		if( * (int *) i % 4 != 0)
			continue;
		if( * (int *) i < dseg_lo || * (int *)i > dseg_hi)
			continue;
		temp = contains( * (int * )i,(Tree **)(dseg_lo + sizeof(long)));
		alloc_tree = * (Tree **) (dseg_lo+sizeof(long));
		if (temp) {
			temp->size = temp->size | 0x01;
			check_allocated_heap(temp);
  ...
  {	
	  tmp_t = (ptr_t * ) pop_stack();
	  verify_garbage((char *) tmp_t + sizeof(Tree));

	  * (Tree **) (dseg_lo + sizeof(long)) = delete(tmp_t, alloc_tree);
	  alloc_tree = * (Tree **) (dseg_lo + sizeof(long));
	  gc_add_free((char * )dseg_lo, (list_t * )tmp_t);
  }
