...
void Comp::exp_indent()
{
    report_error("Comp:: expected indent");
}


void Comp::exp_newline()
{
    report_error("expected newline");
}


void Comp::exp_open_paren()
{
    report_error("expected open paren");
}


void Comp::exp_open_brace()
{
...
void Comp::exp_statement()
{
    report_error("expected statement");
}


void Comp::exp_method_or_field_selector()
{
...
//
bool Comp::compile_file(FILE *file, bool interactive)
{
...
bool Comp::compile(char *filename)
{
    ...
    if (token->typ == TOKEN_ID) {
        if (streql(token->str, "def")) {
            ...
            if (parse_stmt_2() && semantics.method_end()) {
                ...
                Symbol * s_imdd = Symbol::make("<immediate command>", machine);
                machine->global_function(s_imdd);
...
bool Comp::parse_formals()
{
    ...
    if (streql(token->str, ")")) {
        ...
        exp_close_paren_or_formal();
        return false();
    }
}


int Comp::hex_to_int(char h)
{
...
int64 Comp::string_to_int(char *s)
{
	int64 result = 0;
	int sign = 1;
...
bool Comp::parse_default_value()
{
    ...
    if (token->typ == TOKEN_LONG) {
        ...
        return semantics.default_value(
            FDouble::make(atof(token->str), machine));
	} else if (token->typ == TOKEN_STRING) {
        return semantics.default_value(
            FString::make(token->str + 1, machine));
    } else if (token->typ == TOKEN_SYMBOL) {
        return semantics.default_value(
            Symbol::make(token->str + 1, machine));
	} else return false;
}


bool Comp::parse_stmt()
{
    if (get_token()) {
        return parse_stmt_2();
    }
...
/*
bool Comp::parse_assign_id(char *id)
{
...
//
bool Comp::parse_post_inner(char *id)
{
    if (get_token()) {
        if (streql(token->str, ".")) {
...
//
bool Comp::parse_id_expr(char *id)
{
...
bool Comp::parse_id_prod_expr(char *id)
{
...
bool Comp::parse_id_inner_expr(char *id)
{
